<template>
  <div class="payResult">
    <div class="blank" v-if="success == 1"></div>
    <div class v-else-if="success == 2">
      <img class="icon" :src="ok" />
      <p class="success">{{ successContent }}</p>
      <!--<p class="back">{{count}}s后返回</p>-->
      <div class="direction" v-if="serviceTypeId == 3">
        <p>
          <span
            >{{ doctorData.doctorName }} {{ doctorData.academicTitle }}</span
          >
          将为您提供心电解读服务
        </p>
        <p>心电解读报告将在2小时内发给您</p>
      ...
      <div class="doctor_intro" @click="toDoctorDetails" v-if="serviceTypeId == 3">
        ...
        <div class="doctor_introduction">
          <div class="doctor_name">
            ...
            <span>{{ doctorData.academicTitle }}</span>
          </div>
    ...
    <div class v-else-if="success == 4">
      ...
      <!--<p class="fail_reason">支付中...</p>-->
      <!--<p class="back">{{count}}s后返回</p>-->
    </div>
    <div class="wait_btn" v-if="success == 4">
      <p>
        <span @click="toHome">返回首页</span>
      </p>
      <p>
        <span @click="toOrderList">查看订单</span>
      </p>
    </div>
    <div class="to_homePage" @click="toHome" v-else>返回首页</div>
  </div>
</template>
    ...
    consult() {
      this.$router.push({
        ...
        params: {
          type: 1,
          docId: this.docId,
          docName: this.doctorData.doctorName
        }
      });
    },
    toHome() {
