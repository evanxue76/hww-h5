  ...
  <div class="ecg_report">
    ...
    <div class="ecgReport">
      <div class="history_content">
        <div class="scoreResult" v-if="datas.healthLevel == -1">
          干扰数据，请重新测量
        </div>
        <div v-else class="scoreResult">
          {{ datas.ecgElectrocardioDetail.ecgResultBrief }}
        </div>
        <div class="scoreResult">
          <van-icon class="iconLike" name="like" />
          <span>{{ datas.heartRate }}bpm</span>
        </div>

        <div class="ecgPic" @click="viewDetails">
          <img :src="datas.fileImagePath" />
        </div>
        <div class="recordTime">
          <div>{{ datas.takeTime | formatDateFilter }}</div>
          <div>时长：{{ datas.length }}s</div>
        </div>
      </div>
    </div>
    <!-- 心电测量结果 -->
    <div
      ...
      v-if="datas.ecgElectrocardioDetail.ecgLevel != 0"
    >
    ...
    <div class="heartHealth">
      <div class v-if="datas.healthLevel == -1">
        ...
        <div class="ecg_content ecg_result">
          {{ datas.ecgElectrocardioDetail.ecgResultTZ }}
        </div>
      </div>
      ...
      <div class v-else>
        ...
        <div class="ecg_content pathogeny">
          ...
          <div class="analysisResults">
            {{ datas.ecgElectrocardioDetail.abnorAnalysis }}
          </div>
        </div>
        ...
        <div class="ecg_content pathogeny">
          ...
          <div class="analysisResults">
            {{ datas.ecgElectrocardioDetail.suggestions }}
          </div>
        </div>
        ...
        <div class="ecg_content pathogeny">
          ...
          <div class="analysisResults">
            {{ datas.ecgElectrocardioDetail.core }}
          </div>
        </div>
      </div>
    ...
    <div class="footer" v-if="datas.healthLevel != -1">
      <div
        @click="askDoctor"
        class="footMode"
        v-if="datas.ecgElectrocardioDetail.state == 0"
      >
        <span class="foot_pic">
      ...
      <div
        ...
        v-if="datas.ecgElectrocardioDetail.state == 2"
      >
        <span class="foot_pic">
          <img class="foot_icon" :src="doctoranaly" />
        </span>
      ...
      <div
        @click="searchResult"
        class="footMode"
        v-if="datas.ecgElectrocardioDetail.state == 3"
      >
        <span class="foot_pic">
          <img class="foot_icon" :src="doctoranaly" />
        </span>
...
export default {
  ...
  methods: {
    ...
    viewDetails() {
      document.title = "心电图";
//    window.location.href = `http://122.225.207.105:8080/pacs/h5chart/view?file_id=${this.anaecgFileId}`;
      window.location.href = `http://api.995120.cn/pacs/h5chart/view/?file_id=${this.anaecgFileId}`;
    },
    obtionHeartReport() {
      let self = this;
      let Url = "";
      ...
      util.ajax.post(Url, params).then(function(res) {
          self.$toast.clear();
          if (res.data.respHead.respCode == "000") {
            self.datas = res.data.body.data;
            self.anaecgFileId = res.data.body.data.anaecgFileId;
            if (self.datas.healthLevel == 1 || self.datas.healthLevel == 2) {
              self.analysisShow = false;
            } else {
              self.analysisShow = true;
            }
            if (self.datas.ifPayment == 0) {
    ...
    askDoctor() {
      let ecg_data = JSON.stringify(this.datas);
  ...
  computed: {
    ...
    heartStyle() {
      let hrvValue = this.datas.ecgElectrocardioDetail.hrvValue;
      return { left: `${hrvValue / 6 - 2.5}%` };
    },
    pressStyle() {
      let hdriskValue = this.datas.ecgElectrocardioDetail.hdriskValue;
      return { left: `${hdriskValue - 2.5}%` };
    },
    fatigueStyle() {
      let fatigueValue = this.datas.ecgElectrocardioDetail.fatigueValue;
