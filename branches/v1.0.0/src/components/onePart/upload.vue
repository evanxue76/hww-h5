<template>
  <div class="upload">
    ...
    <footer class="next-step">
      <a @click="nextStep" v-if="type==3" style="display:none;"></a>
      <a @click="nextStep" v-else-if="type==2">保存</a>
      <a @click="nextStep" v-else>下一步</a>
    </footer>
  ...
  methods: {
    getInfo() {
      ...
      util.ajax.post(Url, params).then(res => {
        ...
        self.infoSelected.sex == 0 ? (self.infoSelected.sex = "男") : (self.infoSelected.sex = "女");
      });
    },
    imInfo() {
      let self = this;
    ...
    onRead(file) {
      ...
      util.ajax.post(Url, formData).then(function(res) {
