<template>
  <div class="applyUnscramble">
    ...
    <div class="applyServe">
      <van-cell-group>
        <van-cell title="服务机构" :value="data.serveMechanism" />
      </van-cell-group>
      <div class="fuwu_title">服务类型</div>
      <div class="fuwu_box">
        <div
          class="item"
          v-for="(item, index) in fuwu_data.serviceList"
          :key="index"
          @click="toggle_fuwu_active(item)"
          :class="{ fuwu_active: item.is_active }"
        >
    ...
    <div class="footer">
      <div class="totalPrice">
        总计：
        <span class="price">{{ fuwu_data_price }}</span>
      </div>
...
//    let Url = "http://api.995120.cn/hww/hwpay/createOrder";
      let params = {
        ...
        body: {
          ...
          requestStyle: "MWEB",
          serviceTypeId: this.fuwu_item.serviceType,
          type: obj[this.fuwu_item.serviceType],
      ...
      try {
        ...
        console.log(res);
        if (res.data.respHead.respCode === "000") {
        	if (res.data.body.data.pay == 'true') {
        		window.location.href = res.data.body.data.mweb_url + '&redirect_url=http%3a%2f%2fapi-test.995120.cn%2fhwh%2fpayResult?type=2%26orderId=' + res.data.body.data.orderId
        	}else{
        		self.$router.push({
        			name: 'HomePage'
        		})
...
//      }
  ...
  computed: {
    ...
    fuwu_data_price() {
      let num = (this.fuwu_data.price - this.fuwu_data.discount)/100.00;
      return `￥${num}`;
    }
  }
};
