...
//        let Url = "http://api.995120.cn/hww/hwpay/createOrder";
          ...
          utils.ajax.post(Url, params).then(function(res) {
            if (res.data.respHead.respCode == '000') {
            	if (self.types == 1) {
	            	window.location.href = res.data.body.data.mweb_url + '&redirect_url=http%3a%2f%2fapi-test.995120.cn%2fhwh%2fpayResult?type=4%26orderId=' + res.data.body.data.orderId;
	            }else{
	            	window.location.href = res.data.body.data.mweb_url + '&redirect_url=http%3a%2f%2fapi-test.995120.cn%2fhwh%2fpayResult?type=6%26orderId=' + res.data.body.data.orderId;
	            }
