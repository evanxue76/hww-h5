<template>
  <div class="applyUnscramble">
    <div class="ecgReport">
      <div class="history_content">
        <div class="recordTime">
        	<div v-if="ecg_data.ecgElectrocardioDetail.examine == -1">干扰数据，请重新测量</div>
          <div v-else>{{ ecg_data.ecgElectrocardioDetail.ecgResultBrief }}</div>
          <div></div>
        </div>
    ...
    <div class="services_need">
    	<p>服务须知：</p>
    	<p>1.订单完成支付后，问诊时长为24小时，24小时后订单自动结束；</p>
    	<p>2.问诊过程如果问题已解决，医生与用户确认后可提前结束问诊。</p>
    </div>
    <div class="footer">
...
export default {
  ...
  created() {
  	...
  	let host = globals.developmentUrl.host1
		let obj = {
			'http://192.168.9.136:2010/hww/': 'http://192.168.9.136:2010/hww/hwpay/createOrder/',
			'http://api-test.995120.cn/hww/': 'http://api-test.995120.cn/hww/hwpay/createOrder/',
			'http://api.995120.cn/hww/': 'http://api.995120.cn/hww/hwpay/createOrder/'
		}
...
//        infoId: infoId, // 12
      ...
      try {
        let res = await utils.ajax.post(this.baseUrl, params);
        console.log(res);
        if (res.data.respHead.respCode === "000") {
