<template>
  <div class="order">
      ...
      <van-cell-group class="order-list" v-for="(item, index) in orderList" :key="index">
        <div class="order-list-info">
          ...
          <span class="imgSTAT">
            ...
            <img :src="impPic.ecgPic" alt="心电解读">
          </span>
    ...
    <van-popup v-model="show5">
      ...
      <div class="pay_btn" v-if="detalisData.status == 0">
      	<span @click="payAgain(detalisData)">立即支付</span>
      </div>
    </van-popup>
  </div>
</template>
...
export default {
  data() {
    return {
      ...
      impPic: {
        ...
        ecgPic: require("../../assets/image/icon_ecg.png")
      },
      noDataPic: require('../../assets/image/pic_nones.png'),
      detalisData: {},
      hw_user_message: {},
      baseUrl: '',
      baseUrls: ''
  ...
  created() {
  	let host = globals.developmentUrl.host1
		let obj = {
			'http://192.168.9.136:2010/hww/': 'http://api-test.995120.cn/hww/hwpay/repay',
			'http://api-test.995120.cn/hww/': 'http://api-test.995120.cn/hww/hwpay/repay',
			'http://api.995120.cn/hww/': 'http://api.995120.cn/hww/hwpay/repay'
		}
		this.baseUrl = obj[host]
		let objs = {
			'http://192.168.9.136:2010/hww/': 'http%3a%2f%2fapi-test.995120.cn%2fhwh%2fpayResult',
			'http://api-test.995120.cn/hww/': 'http%3a%2f%2fapi-test.995120.cn%2fhwh%2fpayResult',
			'http://api.995120.cn/hww/': 'http%3a%2f%2fapi.995120.cn%2fstatic%2fhm505%2fhwh%2fpayResult'
		}
		this.baseUrls = objs[host]
...
//        let Url = "http://api.995120.cn/hww/hwpay/repay";
          ...
          utils.ajax.post(Url, params).then(function(res) {
            ...
            if (res.data.respHead.respCode == "000") {
            	self.$toast.clear()
            	window.location.href = res.data.body.data.mweb_url + '&redirect_url=' + self.baseUrls + '?docId=' + res.data.body.data.docId + '%26orderId=' + res.data.body.data.orderId + '%26serviceTypeId=' + res.data.body.data.serviceTypeId
...
.order .status-detail-content div{
	height: 24px;
}
...
.order .van-hairline--top-bottom:last-child::after, .order .van-hairline-unset--top-bottom:last-child::after{
	border-width: 0 0;
}
</style>
